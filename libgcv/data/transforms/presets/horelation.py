...
class HORelationDefaultTrainTransform(object):
    ...
    def __call__(self, src, label, box, pose):
        ...
        return img, bbox.astype('float32'), objbox.astype('float32'), pose.astype('float32')
...
class HORelationDefaultVisTransform(object):
    ...
    def __call__(self, src, label, box, pose):
        ...
        img = mx.nd.image.to_tensor(img)
        img = mx.nd.image.normalize(img, mean=self._mean, std=self._std)
        return img, bbox.astype('float32'), objbox.astype('float32'), pose.astype('float32'), ori_img
